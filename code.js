var aegis128L = (function () {
  ...
  const nonceBytes = 16;
  const tagBytes = 16;
  const const0 = new Uint32Array([
    0x02010100, 0xd080503, 0x59372215, 0x6279e990,
  ]);
  const const1 = new Uint32Array([
  ...
  function process(key, nonce, msg, data, decrypt) {
    ...
    // Generate tag
    const tag64 = new BigUint64Array([
      BigInt(dataLen) * 8n,
      BigInt(msgLen) * 8n,
    ]);
  ...
  function open(key, nonce, msg, data, tag) {
    ...
    // console.log("tagDecrypt:", tagDecrypt);
    if (!tagValid(tag, tagDecrypt)) {
      msg.fill(0);
      // return false and tagDecrypt empty
      return false;
    }
    return true;
  }
  ...
  function benchEncrypt() {
    ...
    console.log(
      `Aegis encrypt: Bytes per second: ${
        (n * msg.byteLength) / (d.duration / 1000)
      }`
    );
    console.log(
      `Aegis encrypt: Nanoseconds per byte: ${
        (d.duration * 1000000) / (n * msg.byteLength)
      }`
    );
  }
  ...
  function benchDecrypt() {
    ...
    console.log(
      `Aegis decrypt: Bytes per second: ${
        (n * msg.byteLength) / (d.duration / 1000)
      }`
    );
    console.log(
      `Aegis decrypt: Nanoseconds per byte: ${
        (d.duration * 1000000) / (n * msg.byteLength)
      }`
    );
  }
...
// aegis128L.unitTest();
// aegis128L.benchmarks();
// aegis128L.testKnownAnswers();

// Prepare key and nonce
let key = new Uint8Array([
  0x10, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00,
]);
let nonce = new Uint8Array([
  0x10, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00,
]);

// Convert the message "hello" to Uint8Array
const message = new TextEncoder().encode("hello");
console.log("Message:", message);

// Encrypt the message
const data = new Uint8Array([]);
const tag = aegis128L.seal(key, nonce, message, data);

console.log("Message:", message);

let ciphertext = message.slice();

console.log("Ciphertext:", ciphertext);
// Extract the tag from the ciphertext
const result = aegis128L.open(key, nonce, ciphertext, data, tag);
console.log("Decryption success:", result);
console.log("Decrypted message:", ciphertext);

// Decrypt the message
const decryptedMessage = new TextDecoder().decode(ciphertext);
